...
@iqthon.on(admin_cmd(pattern="كينا(?: |$)(.*)"))
async def kanna(event):
    ...
    if not text:
        if event.is_reply and not reply_to_id.media:
            ...
            await event.edit("قم برد على الرساله")
            return
    await event.edit("جاري كتابه النص الخاص بك ...")
    text = deEmojify(text)
...
@iqthon.on(admin_cmd(pattern="تويتر(?: |$)(.*)"))
async def tweet(event):
    ...
    if not text:
        if event.is_reply:
            if not reply_to_id.media:
                ...
                await event.edit("قم بجلب معرفك ثم قم بالرد عليه بلأمر : .تويتر")
                return
        else:
            await event.edit("قم بجلب معرفك ثم قم بالرد عليه بلأمر : .تويتر")
            return
    if "." in text:
        ...
        await event.edit("قم بجلب معرفك ثم قم بالرد عليه بلأمر : .تويتر")
    await event.edit(f"جاري عمل تغريده الخاصه ب : {username} أنتضر قليلا ...")
    text = deEmojify(text)
